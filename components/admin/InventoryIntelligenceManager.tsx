...
export function InventoryIntelligenceManager({
  ingredients,
  products,
  windowDays,
}: Props) {
  ...
  useEffect(() => {
    setIngredientDrafts(buildIngredientDrafts(ingredients));
  }, [ingredients]);

  useEffect(() => {
    setRecipeDrafts(buildRecipeDrafts(products));
  ...
  const selectedDraft = selectedProduct
    ? recipeDrafts[selectedProduct.id] ?? []
    : [];
