  ...
  return (
    <article className="pisao-image-lift group border-pisao-gold/10 bg-pisao-noche hover:border-pisao-gold/35 flex h-full flex-col overflow-hidden rounded-[1.75rem] border">
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 h-28 bg-linear-to-b from-black/25 to-transparent opacity-70" />
        ...
        <span className="text-pisao-cream absolute right-4 bottom-4 inline-flex translate-y-2 items-center gap-1 text-xs font-semibold opacity-0 transition duration-300 group-hover:translate-y-0 group-hover:opacity-100">
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        ...
        <div className="border-pisao-gold/10 mt-5 flex items-center justify-between gap-3 border-t pt-4">
          <div>
            <p className="text-pisao-cream-muted text-[10px] tracking-[0.14em] uppercase">Precio</p>
            <span className="text-pisao-gold mt-1 block text-base font-semibold">{formatCurrency(product.precio)}</span>
          </div>
          <button
            type="button"
