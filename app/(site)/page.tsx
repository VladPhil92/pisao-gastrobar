    ...
    <>
      ...
      <section className="border-pisao-gold/10 border-y py-6">
        <Container className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          ...
          <Link
            href="/menu"
            className="text-pisao-gold inline-flex items-center gap-2 text-sm font-semibold"
          >
      ...
      <section className="py-20 sm:py-24">
        <Container>
          <div className="max-w-2xl">
            <p className="text-pisao-gold text-xs font-semibold tracking-[0.25em] uppercase">
              ¿Qué plan tienes?
            </p>
            <h2 className="font-display text-pisao-cream mt-3 text-4xl leading-tight sm:text-5xl">
              Una web que te lleva a comer, no a navegar sin rumbo.
            </h2>
          </div>
            ...
            {experiences.map((item) => {
              ...
              const content = (
                <>
                  ...
                  <p className="text-pisao-gold mt-7 text-[11px] font-semibold tracking-[0.2em] uppercase">
                    {item.eyebrow}
                  </p>
        ...
        <Container>
          <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
            <div className="max-w-2xl">
              ...
              <h2 className="font-display text-pisao-cream mt-3 text-4xl sm:text-5xl">
                Comida que entra primero por los ojos.
              </h2>
            </div>
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {signaturePlates.map((plate, index) => (
              <Link
                href="/menu"
                key={plate.src}
                className={`group relative overflow-hidden rounded-3xl ${index === 1 ? "md:translate-y-8" : ""}`}
              >
                <div className="relative aspect-[4/5]">
                  ...
                  <div className="absolute right-0 bottom-0 left-0 p-6">
                    <p className="font-display text-pisao-cream text-2xl">
                      {plate.label}
                    </p>
                    <span className="text-pisao-gold mt-2 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wider">
        ...
        <Container className="grid gap-10 lg:grid-cols-[1.05fr_.95fr] lg:items-center">
          <div>
            <div className="border-pisao-gold/15 bg-pisao-noche inline-flex items-center gap-2 rounded-full border px-4 py-2">
              ...
              <span className="text-pisao-cream-muted text-xs font-semibold uppercase tracking-wider">
                Concierge IA PISÁO
              </span>
            </div>
            ...
            <div className="mt-7 flex flex-wrap gap-2 text-xs">
              {["Recomendaciones", "Presupuesto", "Reservas", "Domicilios", "Eventos"].map(
                (item) => (
                  <span
                    key={item}
                    className="border-pisao-gold/15 text-pisao-cream-muted rounded-full border px-3 py-2"
                  >
                    {item}
                  </span>
                ),
            ...
            <p className="text-pisao-gold mt-7 text-sm font-semibold">
              Ábrelo desde el botón “¿Qué se te antoja?” en la esquina inferior.
            </p>
          </div>
      ...
      <section className="bg-pisao-carbon-soft/35 py-16 sm:py-20">
        <Container className="grid gap-8 lg:grid-cols-[.85fr_1.15fr] lg:items-center">
          ...
          <div className="lg:pl-8">
            <p className="text-pisao-gold text-xs font-semibold tracking-[0.24em] uppercase">
              PISÁO donde estés
            </p>
            <h2 className="font-display text-pisao-cream mt-3 text-4xl sm:text-5xl">
              El plan también puede llegar a tu casa.
            </h2>
            ...
            <div className="border-pisao-gold/25 mt-6 inline-flex items-baseline gap-3 rounded-2xl border px-5 py-4">
              <span className="font-display text-pisao-gold text-4xl">
